...
import org.shutiak.lab4.controllers.BookRestController;
...
public class BookRestControllerTest {
    ...
    void shouldHandleSearchRequest(final String search){
        ...
        when(bookService.getSearchedBooks(search)).thenReturn(expectedBooks);
        mockMvc.perform(
                MockMvcRequestBuilders.get("/books?search="+search)
        )
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(expectedBooks)));
        verify(bookService,times(1)).getSearchedBooks(search);
